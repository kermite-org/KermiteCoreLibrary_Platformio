...
__USBDeviceAttributes __usb_device_attrs = {
  USBD_VID,       //vid
  USBD_PID,       //pid
...
const uint8_t *tud_descriptor_device_cb(void) {
  bool isSerialOnly = __USBInstallSerial && !__USBInstallKeyboard && !__USBInstallMouse && !__USBInstallJoystick && !__USBInstallMassStorage && !__USBInstallSecondHID_RawHID;
...
void __SetupUSBDescriptor() {
  if (!usbd_desc_cfg) {
    ...
    // uint8_t hid2_itf = __USBInstallSerial ? 3 : 0;
    // uint8_t msd_itf = interface_count - 1;
    uint8_t msd_desc[TUD_MSC_DESC_LEN] = {
      TUD_MSC_DESCRIPTOR(itf_msd, 0, USBD_MSC_EPOUT, USBD_MSC_EPIN, USBD_MSC_EPSIZE)
    };
    ...
    int usbd_desc_len =
        TUD_CONFIG_DESC_LEN +
        (hasSerial ? sizeof(cdc_desc) : 0) +
        (hasHID ? sizeof(hid_desc) : 0) +
        (hasMSD ? sizeof(msd_desc) : 0) +
